...
import { getuBASEDPrice, getUSDRate } from 'pricing'
...
function getMasterChef(block: ethereum.Block): MasterChef {
  ...
  if (masterChef === null) {
    ...
    masterChef.ubased = contract.ubased()
    masterChef.ubasedPerBlock = contract.ubasedPerBlock()
...
export function getPool(id: BigInt, block: ethereum.Block): Pool {
  ...
  if (pool === null) {
    ...
    pool.accuBASEDPerShare = poolInfo.value3
    ...
    pool.ubasedHarvested = BIG_DECIMAL_ZERO
    pool.ubasedHarvestedUSD = BIG_DECIMAL_ZERO
    pool.save()
  }
...
function getPoolHistory(pool: Pool, block: ethereum.Block): PoolHistory {
  ...
  if (history === null) {
    ...
    history.ubasedHarvested = BIG_DECIMAL_ZERO
    history.ubasedHarvestedUSD = BIG_DECIMAL_ZERO
  }
...
export function getUser(pid: BigInt, address: Address, block: ethereum.Block): User {
  ...
  if (user === null) {
    ...
    user.ubasedHarvested = BIG_DECIMAL_ZERO
    user.ubasedHarvestedUSD = BIG_DECIMAL_ZERO
...
export function deposit(event: Deposit): void {
  ...
  if (event.block.number.gt(MASTER_CHEF_START_BLOCK) && user.amount.gt(BIG_INT_ZERO)) {
    const pending = user.amount
      .toBigDecimal()
      .times(pool.accuBASEDPerShare.toBigDecimal())
      .div(BIG_DECIMAL_1E12)
      .minus(user.rewardDebt.toBigDecimal())
      .div(BIG_DECIMAL_1E18)
    // log.info('Deposit: User amount is more than zero, we should harvest {} ubased', [pending.toString()])
    if (pending.gt(BIG_DECIMAL_ZERO)) {
      // log.info('Harvesting {} SUSHI', [pending.toString()])
      const ubasedHarvestedUSD = pending.times(getuBASEDPrice(event.block))
      user.ubasedHarvested = user.ubasedHarvested.plus(pending)
      user.ubasedHarvestedUSD = user.ubasedHarvestedUSD.plus(ubasedHarvestedUSD)
      pool.ubasedHarvested = pool.ubasedHarvested.plus(pending)
      pool.ubasedHarvestedUSD = pool.ubasedHarvestedUSD.plus(ubasedHarvestedUSD)
      poolHistory.ubasedHarvested = pool.ubasedHarvested
      poolHistory.ubasedHarvestedUSD = pool.ubasedHarvestedUSD
    }
  }
...
export function withdraw(event: Withdraw): void {
  ...
  if (event.block.number.gt(MASTER_CHEF_START_BLOCK) && user.amount.gt(BIG_INT_ZERO)) {
    const pending = user.amount
      ...
      .minus(user.rewardDebt.toBigDecimal())
      .div(BIG_DECIMAL_1E18)
    // log.info('Withdraw: User amount is more than zero, we should harvest {} ubased - block: {}', [
    //   pending.toString(),
    //   event.block.number.toString(),
    // ])
    // log.info('SUSHI PRICE {}', [getuBASEDPrice(event.block).toString()])
    if (pending.gt(BIG_DECIMAL_ZERO)) {
      // log.info('Harvesting {} SUSHI (CURRENT SUSHI PRICE {})', [
      //   pending.toString(),
      //   getuBASEDPrice(event.block).toString(),
      // ])
      const ubasedHarvestedUSD = pending.times(getuBASEDPrice(event.block))
      user.ubasedHarvested = user.ubasedHarvested.plus(pending)
      user.ubasedHarvestedUSD = user.ubasedHarvestedUSD.plus(ubasedHarvestedUSD)
